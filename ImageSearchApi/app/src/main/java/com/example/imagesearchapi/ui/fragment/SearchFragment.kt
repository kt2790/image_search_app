...
class SearchFragment : Fragment() {
    ...
    private fun initView() {
        ...
        binding.confirmBtn.setOnClickListener {
            viewModel.getImageList(binding.searchText.text.toString(), "accuracy")
        }
        ...
        viewModel.searchResult.observe(viewLifecycleOwner) {
            when(it) {
                is ApiState.Loading -> {}
                is ApiState.Success -> {
                    val imageSearchResponse = it.data as ImageSearchResponse
                    adapter.setList(imageSearchResponse.documents ?: mutableListOf())
                }
                is ApiState.Error -> {}
            }
        }
